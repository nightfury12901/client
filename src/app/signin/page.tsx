...
export default function SigninPage() {
  ...
  const handleGoogleSignin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/api/auth/callback?next=/app/leads` },
    });
  ...
  return (
    <div className={styles.page}>
      <div className={styles.formPane}>
        ...
        <form className={styles.formCard} onSubmit={handleEmailSignin}>
          ...
          <input className={styles.input} type="email" name="email" placeholder="Email address" value={form.email} onChange={handleChange} autoComplete="email" required/>

          <div className={styles.passWrap}>
            <input className={styles.input} type={showPass ? 'text' : 'password'} name="password" placeholder="Password" value={form.password} onChange={handleChange} autoComplete="current-password" required/>
          ...
          <div style={{ textAlign: 'right', marginBottom: 16 }}>
            <Link href="/forgot-password" className={styles.link} style={{ fontSize: 12 }}>Forgot password?</Link>
          </div>
      ...
      <div className={styles.decoPane}>
        <div className={styles.decoArt}>
          <div className={`${styles.floatCard} ${styles.floatCard1}`}>
            ...
            <div><p className={styles.floatCardVal}>8,400+</p><p className={styles.floatCardLbl}>active freelancers</p></div>
          </div>
          <div className={styles.decoCenterIll}>
            <svg viewBox="0 0 200 220" fill="none" className={styles.decoSvg}>
              <circle cx="100" cy="44" r="20" stroke="rgba(240,240,240,0.1)" strokeWidth="1"/>
              <path d="M80 66 L72 104 L128 104 L120 66 Z" stroke="rgba(240,240,240,0.07)" strokeWidth="1" fill="none"/>
              ...
              <rect x="38" y="28" width="6" height="6" rx="1" fill="rgba(240,240,240,0.12)"/>
              <rect x="156" y="28" width="6" height="6" rx="1" fill="rgba(240,240,240,0.12)"/>
              <rect x="38" y="186" width="6" height="6" rx="1" fill="rgba(240,240,240,0.12)"/>
              <rect x="156" y="186" width="6" height="6" rx="1" fill="rgba(240,240,240,0.12)"/>
        ...
        <p className={styles.decoTagline}>Your next client<br/><em>is one proposal away.</em></p>
      </div>
    </div>
  );
}
